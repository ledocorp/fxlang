...
On [0.9.2](0.9.2.md): four production surfaces — native testing culture, structured concurrency facades, stronger std packs, and capability / guest regions with IR dual-path — plus vendor checksum foothold.
...
| **Concurrency facades** | `std/nursery`, `std/chan`, `std/select`, `std/mailbox`, `std/supervise` + `host/concur` (emit-C and IR when linked) |
