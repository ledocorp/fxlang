/* FsCap / OutCap + guest session runtime (stdio). */
...
static void guest_unlink_from_parent(FxGuestCtx *g) {
    FxGuestCtx *p;
    int i;
    ...
    for (i = 0; i < p->children_n; i++) {
        if (p->children[i] == g) {
            p->children[i] = p->children[p->children_n - 1];
            p->children[p->children_n - 1] = NULL;
            p->children_n--;
            break;
        }
    }
...
int fx_guest_end(int64_t ctx_handle) {
    ...
    /* Parent end tears down nested children first (session graph). */
    while (g->children_n > 0) {
        FxGuestCtx *child = g->children[0];
        if (child == NULL) {
            g->children_n = 0;
            break;
        }
        (void)fx_guest_end(fx_guest_to_handle(child));
    }
    guest_unlink_from_parent(g);
    ...
    g->arena_used = 0;
    g->live = 0;
    /* Cap + ctx stay as live==0 tombstones so stale handles / is_live / nested
     * under a dead parent deny safely. Arena memory is released. */
    clear_last_text();
    return 0;
}
...
FxNetCap *fx_netcap_from_handle(int64_t handle) {
    return (FxNetCap *)(intptr_t)handle;
}

int fx_netcap_host_port_allowed(const FxNetCap *cap, const char *host, int port) {
    if (cap == NULL || !cap->live || host == NULL || host[0] == '\0') {
        return 0;
    }
    if (port < cap->port_min || port > cap->port_max) {
        return 0;
    }
    return strcmp(cap->allow_host, host) == 0 ? 1 : 0;
}

int fx_netcap_allows(int64_t net_handle, const char *host, int port) {
    return fx_netcap_host_port_allowed(fx_netcap_from_handle(net_handle), host, port);
}

/* Mint NetCap (allowlist only; no dial). */
int64_t fx_guest_mint_netcap(int64_t ctx_handle, const char *allow_host, int port_min,
    ...
    if (allow_host == NULL || allow_host[0] == '\0') {
        return 0;
    }
    if (port_min < 1 || port_max > 65535 || port_min > port_max) {
        return 0;
    }
    if (g->net_n >= FX_GUEST_MAX_CAPS) {
        return 0;
    }
    n = strlen(allow_host);
    if (n >= sizeof(((FxNetCap *)0)->allow_host)) {
        return 0;
    }
    cap = (FxNetCap *)calloc(1, sizeof(FxNetCap));
    if (cap == NULL) {
        return 0;
    }
    memcpy(cap->allow_host, allow_host, n + 1);
    cap->port_min = port_min;
    cap->port_max = port_max;
    cap->live = 1;
    g->net[g->net_n++] = cap;
    return fx_netcap_to_handle(cap);
}

/* Nested guest session under a live parent. */
int64_t fx_guest_begin_nested(int64_t parent_ctx, const char *root, int64_t arena_bytes) {
    ...
    parent = fx_guest_from_handle(parent_ctx);
    if (parent == NULL || !parent->live) {
        return 0;
    }
    if (parent->children_n >= FX_GUEST_MAX_CHILDREN) {
        return 0;
    }
    ...
    if (root != NULL && root[0] != '\0') {
        use_root = root;
    } else if (parent->root[0] != '\0') {
        use_root = parent->root;
    }
    if (use_root != NULL) {
        if (!mint_root(g->root, sizeof(g->root), use_root)) {
            free(g);
            return 0;
        }
    }
    ...
    g->parent = parent;
    g->live = 1;
    parent->children[parent->children_n++] = g;
    return fx_guest_to_handle(g);
}
