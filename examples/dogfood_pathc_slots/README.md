# Slot-distance dogfood - dense slot distances

Sample: **grow once**, then **no-grow** `v[i]=x` slot updates under `effects { mut }`.

| Phase | API |
|-------|-----|
| Grow | `vec.push` / reassign |
| Slot write | `dist[i] = x` (MUT-2 sugar; same family as `vec_set`) |
| Not supported | Growable realloc under index-assign |
