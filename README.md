...
Named **regions** + **effects** in signatures. **Native-first** `fx run` (live `sh_*` when the program fits; otherwise a built-in fallback engine inside `bin/fx`). **IR â†’ native** on that fallback path when QBE is present. **Emit-C** stays first-class and readable. Wrap mature C libraries instead of rewriting them.

This repo is the **language package**: put `bin/` on your `PATH` and write fx.  
**License:** GNU GPL v3 â€” see [LICENSE](LICENSE). No Rust toolchain required.

---

## Install / try (2 minutes)

**Needs:** this repo Â· `gcc` (or clang/zig) Â· **Windows or Linux x86_64** (no macOS prebuilt).
...
**Argv:** `fx run` does not forward program args â€” use `--scaffold cli` / `--host`.  
...
| Map | [docs/SURFACE.md](docs/SURFACE.md) â€” as-implemented cheatsheet |
| Tracking | [docs/TRACKING.md](docs/TRACKING.md) |
| Std / Reference | [docs/STD.md](docs/STD.md) Â· [docs/REFERENCE.md](docs/REFERENCE.md) |

Whatâ€™s next (honest): [docs/NEXT.md](docs/NEXT.md).
...
- **Wider live demos** â€” no-grow slot dogfood, tools, composition, concur footholds, `manifest_gate`, `std_path_smoke`, and related std surfaces
...
- `fx doctor` Â· `fx lsp` Â· `fx mcp` (check / locate / run)
